...
create or replace package body app_json as 
...
begin
   if app_json.does_json_data_path_exist (
      p_json_key=>p_json_key,
      p_json_path=>p_error_path) then 
      if p_error_type_path is not null then 
         v_error_type := app_json.get_json_data_string(p_json_key=>p_json_key, p_json_path=>p_error_type_path);
      end if;
      if p_error_message_path is not null then 
         v_error_message := app_json.get_json_data_string(p_json_key=>p_json_key, p_json_path=>p_error_message_path);
      end if;
...
procedure json_to_data_table_handle_array (
   ...
   v_data_size number;
begin 
   debug2('json_to_data_table_handle_array: '||p_json_key);
...
begin
   ...
   for key in 1..k.count loop
      ...
      debug2('name='||v_data_key||', size='||v_data_size);
...
exception
   when others then
      log_err('json_to_data_table: '||p_depth||', '||p_json_path||', '||sqlerrm);
      raise;
end;
...
function get_clob_from_store (
   ...
   r clob;
begin 
   return app_json.to_clob(p_json=>get_json_from_store(p_json_key=>p_json_key));
end;
