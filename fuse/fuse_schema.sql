...
begin 
   if not does_sequence_exist('seq_session_prompt_id') then
      execute immediate 'create sequence seq_session_prompt_id';
   end if;
end;
/
...
begin
   if not does_table_exist('session_prompt') then 
      ...
      session_prompt_id number not null,
...
exec drop_table('json_response');

begin
   if not does_table_exist('json_response') then 
      execute immediate '
      create table json_response (
      json_response_id number generated by default on null as identity cache 20 noorder nocycle nokeep noscale not null,
      response json not null,
      session_prompt_id number default null,
      created timestamp default systimestamp)';
   end if;
   add_primary_key('json_response', 'json_response_id');
   if not does_constraint_exist('fk_json_response_session_prompt_id') then
      execute immediate 'alter table json_response add constraint fk_json_response_session_prompt foreign key (session_prompt_id) references session_prompt(session_prompt_id) on delete cascade';
   end if;
end;
/

comment on table json_response is 'Each row contains the raw json post/response of an API call.';
